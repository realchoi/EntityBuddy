...
public static class AbpStringExtensions
{
    ...
    /// <summary>
    /// 判断字符串是否不为空，且是否可以转换成 long 类型。
    /// 上述条件都成立则返回 true，并返回转换后的 long 类型数值；否则返回 false。
    /// </summary>
    /// <param name="str">源字符串</param>
    /// <param name="longVal">转换后的 long 类型数值</param>
    /// <returns></returns>
    public static bool TryParseToLong(this string str, out long? longVal)
    {
        longVal = null;
        if (str.IsNullOrEmpty() || !long.TryParse(str.Trim(), out var num)) return false;
        longVal = num;
        return true;
    }
